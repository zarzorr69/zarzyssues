...
function Start-App {
    ...
    Load-HFToken
    Say "Starting native Odysseus on http://127.0.0.1:$Port"
    ...
    if (Current-Process) { Okay "Server started (PID $($proc.Id)); URL: http://localhost:$Port" }
...
function Enable-Startup {
    if (-not (Test-Path -LiteralPath $ManagedScript)) { Copy-Item -LiteralPath $PSCommandPath -Destination $ManagedScript -Force }
    ...
    while($true){
        ...
        try{
            switch($choice){
                ...
                '12'{Update-Repo};'0'{return};default{Warn 'Unknown choice.'}
            }
