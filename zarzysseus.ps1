...
chromadb-start, ollama-sync, tool-health, agent-tool-focus-install.
#>
[CmdletBinding()]
param(
...
$ProjectDir = $null
# Never install Docker merely to start ChromaDB; use the existing upstream Compose service.
...
function Ensure-Chromadb {
    if (-not $ChromaAutoStart) { Warn 'ChromaDB auto-start disabled by CHROMADB_AUTO_START=0'; return }
    if (Chroma-Ready) { Okay 'ChromaDB heartbeat ready at 127.0.0.1:8100'; return }
    if (-not (Have 'docker')) { Warn 'Docker not installed. Install Docker Desktop with Compose, or configure an existing ChromaDB server.'; return }
    ...
    try {
        ...
        Say 'Starting upstream ChromaDB Compose service (no new Docker installation)'
    ...
    while($true){
        ...
        try{
            switch($choice){
                ...
                '15'{Ensure-Chromadb};'16'{Sync-Ollama};'17'{Tool-Health};
                '18'{Install-QwenToolFocus};'0'{return};default{Warn 'Unknown choice.'}
            }
...
try {
    ...
    switch($Action.ToLowerInvariant()){
        ...
        'ollama-pull'{Pull-Ollama $Argument};'ollama-sync'{Sync-Ollama};'models'{Model-Browser};'skills'{Skills-Menu};
        ...
        'chromadb-start'{Ensure-Chromadb};'tool-health'{Tool-Health};
