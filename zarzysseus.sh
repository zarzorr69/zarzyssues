...
#   ./zarzysseus.sh workspace-default [status]  Set clone as Agent's first-use workspace
#   ./zarzysseus.sh docker-setup  Provision Docker/Compose and ChromaDB explicitly
...
# Host binding is stored outside the upstream Git checkout and survives updates.
NETWORK_HOST_FILE="$HOME/.config/zarzysseus/network-host"
NETWORK_STORED_HOST=""
if [[ -r "$NETWORK_HOST_FILE" ]]; then
    IFS= read -r NETWORK_STORED_HOST < "$NETWORK_HOST_FILE" || true
fi
case "$NETWORK_STORED_HOST" in 0.0.0.0|127.0.0.1) ;; *) NETWORK_STORED_HOST=0.0.0.0 ;; esac
ODYSSEUS_HOST="${ZARZYSSEUS_HOST:-${ODYSSEUS_HOST:-$NETWORK_STORED_HOST}}"
...
# Optional, source-guarded local patch: focus an explicitly named native tool
# for first-round Qwen agent calls. This is NOT an automatic forced retry.
ZARZYSSEUS_QWEN_TOOL_FOCUS="${ZARZYSSEUS_QWEN_TOOL_FOCUS:-1}"
# The discovered/cloned checkout is the first-use browser workspace for Agent Mode.
# A browser-selected workspace (or explicit clear) always takes precedence.
ZARZYSSEUS_DEFAULT_WORKSPACE="${ZARZYSSEUS_DEFAULT_WORKSPACE:-1}"
CHROMADB_HEARTBEAT_URL="${CHROMADB_HEARTBEAT_URL:-http://127.0.0.1:8100/api/v2/heartbeat}"
CHROMADB_AUTO_START="${CHROMADB_AUTO_START:-1}"
ZARZYSSEUS_DOCKER_AUTO_INSTALL="${ZARZYSSEUS_DOCKER_AUTO_INSTALL:-1}"
...
# Network Discoverability controls only the Odysseus web server, not Ollama or
# model-runner endpoints. 0.0.0.0 is a listening address, not a browser URL.
network_lan_ips() {
    if command -v ip >/dev/null 2>&1; then
        ip -o -4 addr show scope global 2>/dev/null | awk '{split($4,a,"/"); if (a[1] !~ /^169\.254\./) print a[1]}' | sort -u
...
show_link() {
    echo "Local URL: http://127.0.0.1:${ODYSSEUS_PORT}"
    if [[ "$ODYSSEUS_HOST" == 0.0.0.0 ]]; then
        echo 'Network Discoverability: LAN / all interfaces (0.0.0.0)'
        local ip
        while IFS= read -r ip; do
            [[ -n "$ip" ]] && echo "  From another device: http://${ip}:${ODYSSEUS_PORT}"
        done < <(network_lan_ips)
        echo 'Network access depends on routing/firewall rules; do not port-forward publicly.'
    else
        echo 'Network Discoverability: Localhost only (127.0.0.1)'
    fi
}

network_status() {
    echo '=== Odysseus Network Discoverability ==='
    echo "Configured listener: $ODYSSEUS_HOST:$ODYSSEUS_PORT"
    show_link
    if [[ -f "$SERVICE_FILE" ]]; then
        local service_host
        service_host="$(sed -nE '/^ExecStart=.*-m uvicorn app:app/ s/.* --host ([^[:space:]]+).*/\1/p' "$SERVICE_FILE" | head -n1)"
        if [[ -n "$service_host" && "$service_host" != "$ODYSSEUS_HOST" ]]; then
            echo "WARNING: Service currently specifies $service_host; choose a mode again to synchronize it."
        fi
    fi
    if [[ -n "${ZARZYSSEUS_HOST:-}${ODYSSEUS_HOST_OVERRIDE:-}" ]]; then
        echo 'Note: a host environment override may supersede the saved mode on next launch.'
    fi
    echo 'Only use LAN mode on a trusted network; firewall and access control remain your responsibility.'
}

network_mode_set() {
    local mode="${1:-}" host
    case "$mode" in
        localhost|local|127.0.0.1) host=127.0.0.1 ;;
        lan|network|0.0.0.0) host=0.0.0.0 ;;
        *) echo 'Usage: zarzysseus.sh network-mode localhost|lan|status'; return 2 ;;
    esac
    mkdir -p "$(dirname "$NETWORK_HOST_FILE")"
    printf '%s\n' "$host" > "$NETWORK_HOST_FILE"
    chmod 600 "$NETWORK_HOST_FILE" 2>/dev/null || true
    ODYSSEUS_HOST="$host"
    echo "Saved listener: $host:$ODYSSEUS_PORT"
    if [[ -f "$SERVICE_FILE" ]]; then
        if ! grep -Eq '^ExecStart=.*-m uvicorn app:app --host [^[:space:]]+' "$SERVICE_FILE"; then
            echo 'WARNING: The service ExecStart is not recognized; retained the saved mode.'
            echo 'Reinstall/update the stack to regenerate the service before assuming it changed.'
            return 1
        fi
        sed -i -E "/^ExecStart=.*-m uvicorn app:app/ s/( --host )[^[:space:]]+/\1${host}/" "$SERVICE_FILE"
        if command -v systemctl >/dev/null 2>&1 && systemctl --user daemon-reload; then
            if systemctl --user is-active --quiet "$service_unit"; then
                echo 'Restarting the running Odysseus service to apply the new listener...'
                systemctl --user restart "$service_unit" || {
                    echo 'ERROR: Listener saved, but service restart failed. Check journalctl --user.'
                    return 1
                }
            fi
        else
            echo 'WARNING: Service file changed but systemd reload was unavailable; apply it manually.'
            return 1
        fi
    else
        echo 'No managed service exists yet; the selected listener will be used on installation.'
    fi
    network_status
}
...
hf_pull_model() {
    ...
    case "${repo,,}" in
        tobestyledintro/qwen3.8-9b-distill|tobestyledintro/qwen3.8-9b-distill:*)
            echo "ERROR: '$repo' is an Ollama registry model, not a Hugging Face repo." >&2
            echo "Use: ./zarzysseus.sh pull '$repo'" >&2
            return 2
            ;;
    esac
...
model_source_and_provider() {
    ...
    # This community repack is in the Ollama registry, NOT a Hugging Face
    # model repository. Its namespace/model syntax must not trigger HF routing
    # on a fresh install before `ollama list` knows the model.
    case "${model,,}" in
        tobestyledintro/qwen3.8-9b-distill|tobestyledintro/qwen3.8-9b-distill:*)
            echo "ollama|$model"
            return 0
...
# Docker is an install-time dependency, not a prerequisite for opening the TUI.
# Never add the user to the docker group automatically (root-equivalent access).
# Prefer the existing user daemon; fall back to sudo/doas for a root-owned daemon.
docker_compose() {
    if docker info >/dev/null 2>&1; then
        ...
        echo '    [warning] Docker daemon unavailable or current user lacks access.' >&2
        return 1
    fi
}

ensure_docker_for_committed_install() {
    local force="${1:-}" rc=0
    if [[ "$force" != force ]]; then
        [[ "$ZARZYSSEUS_DOCKER_AUTO_INSTALL" == 1 && "$CHROMADB_AUTO_START" == 1 ]] || {
            echo '    [note] Docker auto-setup disabled; existing ChromaDB may still be used.'
            return 0
        }
    fi
    if chromadb_heartbeat_ready; then
        echo '    [ready] ChromaDB already available; Docker provisioning unnecessary.'
        return 0
    fi
    if ! command -v docker >/dev/null 2>&1; then
        echo '==> Docker missing; installing the engine via the detected distribution package manager...'
        case "$PKG" in
            pacman) install_missing_packages docker docker-compose || rc=$? ;;
            apt)
                # Debian/Ubuntu package names differ: install engine first, then
                # whichever native Docker Compose v2 plugin exists in its repo.
                $SUDO apt-get update || rc=$?
                if (( rc == 0 )); then install_missing_packages docker.io || rc=$?; fi
                if (( rc == 0 )); then
                    install_missing_packages docker-compose-v2 ||
                        install_missing_packages docker-compose-plugin || rc=$?
                fi
                ;;
            dnf|dnf5|yum|microdnf)
                # Distribution repositories may use docker or moby-engine.
                install_missing_packages docker || install_missing_packages moby-engine || rc=$?
                if (( rc == 0 )); then
                    install_missing_packages docker-compose-plugin ||
                        install_missing_packages docker-compose || rc=$?
                fi
                ;;
            zypper) install_missing_packages docker docker-compose || rc=$? ;;
            apk) install_missing_packages docker docker-cli-compose || rc=$? ;;
            xbps) install_missing_packages docker docker-compose || rc=$? ;;
            emerge) install_missing_packages app-containers/docker app-containers/docker-compose || rc=$? ;;
            *)
                echo "    [warning] No verified automated Docker package recipe for $PKG. Install Docker Engine and Compose v2 using your distro's method."
                return 1 ;;
        esac
        if (( rc != 0 )); then
            echo '    [warning] Docker package install did not complete; rerun after repairing package repositories.'
            return 1
        fi
        hash -r
    fi
    if ! command -v docker >/dev/null 2>&1; then
        echo '    [warning] Docker command still missing; open a new terminal and rerun.'
        return 1
    fi
    if ! docker compose version >/dev/null 2>&1 &&
       { [[ -z "$SUDO" ]] || ! $SUDO docker compose version >/dev/null 2>&1; }; then
        # Existing engine may predate Compose or have an incomplete installation.
        echo '==> Docker Compose v2 missing; attempting installation...'
        case "$PKG" in
            pacman|zypper|xbps) install_missing_packages docker-compose || return 1 ;;
            apt) install_missing_packages docker-compose-v2 || install_missing_packages docker-compose-plugin || return 1 ;;
            dnf|dnf5|yum|microdnf) install_missing_packages docker-compose-plugin || install_missing_packages docker-compose || return 1 ;;
            apk) install_missing_packages docker-cli-compose || return 1 ;;
            emerge) install_missing_packages app-containers/docker-compose || return 1 ;;
            *) echo '    [warning] Install Docker Compose v2 for this distribution.'; return 1 ;;
        esac
    fi
    if ! docker info >/dev/null 2>&1 &&
       { [[ -z "$SUDO" ]] || ! $SUDO docker info >/dev/null 2>&1; }; then
        if [[ "$SERVICE_MANAGER" == systemd ]]; then
            echo '==> Enabling/starting Docker Engine system service...'
            $SUDO systemctl enable --now docker || {
                echo '    [warning] Docker service could not start. Check: systemctl status docker'; return 1;
            }
        elif [[ "$SERVICE_MANAGER" == openrc ]]; then
            $SUDO rc-update add docker default || true
            $SUDO rc-service docker start || {
                echo '    [warning] Docker OpenRC service did not start.'; return 1;
            }
        else
            echo '    [warning] Start the Docker daemon through your service manager, then rerun.'
            return 1
        fi
    fi
    if docker_compose version >/dev/null 2>&1; then
        echo '    [ready] Docker Engine and Compose v2 available (existing access or privilege wrapper).'
        return 0
    fi
    echo '    [warning] Docker/Compose not ready. No insecure Docker group membership was added.'
    return 1
}

# ChromaDB is required for Odysseus's semantic tool index and vector memory.
# Use the checkout's existing compose service; never publish another database
# or start Docker just for a read-only status operation.
chromadb_heartbeat_ready() {
    command -v curl >/dev/null 2>&1 || return 1
    curl -fsS --max-time 3 "$CHROMADB_HEARTBEAT_URL" >/dev/null 2>&1
}

ensure_chromadb_running() {
    [[ "$CHROMADB_AUTO_START" == 1 || "${1:-}" == force ]] || { echo '    [note] ChromaDB auto-start disabled.'; return 0; }
    if chromadb_heartbeat_ready; then
        echo '    [ready] ChromaDB / tool index (127.0.0.1:8100)'
        return 0
    fi
    if ! command -v docker >/dev/null 2>&1 || ! docker_compose version >/dev/null 2>&1; then
        echo '    [warning] Docker Compose unavailable; cannot start Odysseus ChromaDB.'
        return 1
    fi
    if [[ ! -f "$PROJECT_DIR/compose.yaml" && ! -f "$PROJECT_DIR/compose.yml" && ! -f "$PROJECT_DIR/docker-compose.yml" && ! -f "$PROJECT_DIR/docker-compose.yaml" ]]; then
        echo '    [warning] No Odysseus Docker Compose file; ChromaDB not started.'
        return 1
    fi
    if ! (cd "$PROJECT_DIR" && docker_compose config --services 2>/dev/null | grep -Fxq chromadb); then
        echo '    [warning] This checkout has no chromadb Compose service.'
        return 1
    fi
    echo '==> Starting existing Odysseus ChromaDB service...'
    if ! (cd "$PROJECT_DIR" && docker_compose up -d chromadb); then
        echo '    [warning] Docker Compose could not start ChromaDB.'
        return 1
    fi
    local attempt
    for attempt in {1..15}; do
        if chromadb_heartbeat_ready; then
            echo '    [ready] ChromaDB connected (tool index and VectorRAG available)'
            return 0
        fi
        sleep 1
    done
    echo '    [warning] ChromaDB container started, but heartbeat is not responding yet.'
    return 1
}

# Odysseus persists the Agent workspace in browser Storage, NOT in Python
# settings or a systemd WorkingDirectory. Set PROJECT_DIR as a *first-use*
# browser default while leaving existing browser-selected workspaces and explicit
# clears alone. This source-guarded patch is applied after cloning/setup and on
# service starts/restarts; upstream edits cause a warning rather than a blind
# replacement. The native service WorkingDirectory already uses PROJECT_DIR.
ensure_odysseus_default_workspace() {
    [[ "$ZARZYSSEUS_DEFAULT_WORKSPACE" == 1 ]] || { echo '    [note] Default Agent workspace disabled by environment.'; return 0; }
    repo_installed || { echo "ERROR: Odysseus checkout is missing at $PROJECT_DIR"; return 1; }
    local source_file="$PROJECT_DIR/static/js/workspace.js" py=""
    [[ -f "$source_file" ]] || { echo '    [warning] Upstream workspace.js is missing; cannot set browser default.'; return 1; }
    if [[ -x "$VENV_DIR/bin/python" ]]; then
        py="$VENV_DIR/bin/python"
    elif command -v python3 >/dev/null 2>&1; then
        py="$(command -v python3)"
    else
        echo '    [warning] Python is unavailable; cannot configure Agent workspace.'
        return 1
    fi
    "$py" - "$source_file" "$PROJECT_DIR" "$PROJECT_DIR/data/local/patch-backups" <<'PY_WORKSPACE_DEFAULT'
import datetime
import json
import pathlib
import re
...
PY_WORKSPACE_DEFAULT
}

# An opt-in local adaptation for explicitly named Qwen tool requests. It only
# narrows already authorized schemas; it does not execute tools, bypass approval,
# force calls on ordinary chat, or modify Llama/non-native routes. Keep backups
# because Odysseus is upstream code and updates may change the surrounding code.
install_qwen_tool_focus() {
    [[ "$ZARZYSSEUS_QWEN_TOOL_FOCUS" == 1 ]] || { echo '    [note] Qwen tool focus disabled.'; return 0; }
    local source_file="$PROJECT_DIR/src/agent_loop.py"
    [[ -f "$source_file" && -x "$VENV_DIR/bin/python" ]] || {
        echo 'ERROR: Odysseus source and Python environment are required for the Qwen focus patch.'
        return 1
    }
    "$VENV_DIR/bin/python" - "$source_file" "$PROJECT_DIR/data/local/patch-backups" <<'PY_QWEN_FOCUS'
...
# Read-only health report: an advertised Ollama capability is not proof that
# the agent actually executed a native tool, so print both separately.
tool_calling_health() {
    echo '=== Zarzysseus tool-calling health ==='
    if ollama_api_ready; then echo 'Ollama API: ready'; else echo 'Ollama API: UNREACHABLE'; fi
    if chromadb_heartbeat_ready; then echo 'ChromaDB heartbeat: ready'; else echo 'ChromaDB heartbeat: UNREACHABLE'; fi
    if [[ ! -x "$VENV_DIR/bin/python" ]]; then
        echo 'Odysseus venv: not installed'
        return 1
    fi
    ( cd "$PROJECT_DIR" && "$VENV_DIR/bin/python" - <<'PY_TOOL_HEALTH'
...
with SessionLocal() as db:
    for ep in db.query(ModelEndpoint).all():
        ...
        if parsed.hostname in ("localhost", "127.0.0.1") and parsed.port == 11434 and parsed.path.rstrip("/") == "/v1":
            print(f"Ollama endpoint {ep.id}: tools={ep.supports_tools!r} (host={parsed.hostname})")
...
focus = Path("src/agent_loop.py")
print("Qwen explicit-tool focus:", "installed" if focus.is_file() and "ZARZYSSEUS_QWEN_EXPLICIT_TOOL_FOCUS_V1" in focus.read_text(encoding="utf-8") else "not installed")
print("Note: effective per-user/session model routing may differ from global settings.")
PY_TOOL_HEALTH
    )
}
...
service_enable() {
    ...
    ensure_odysseus_default_workspace || echo "    [warning] Could not refresh default Agent workspace."
    ensure_chromadb_running || true
...
install_odysseus() {
    ...
    # This runs only after a committed install/update or confirmed model selection;
    # never on TUI launch, model browsing, or status. Docker setup is repeatable.
    ensure_docker_for_committed_install || echo '    [warning] Docker is not ready; model installation continues, ChromaDB may remain unavailable.'
...
PY
    ...
    ensure_odysseus_default_workspace || echo "    [warning] Default workspace setup needs attention; run: $0 workspace-default"
    ensure_chromadb_running || true
...
tui_full_install_model_select() {
    local -a labels=(
        ...
        'tobestyledintro/qwen3.8-9b-distill | LIGHTER / SMART (OLLAMA)'
...
manual_model_install_batch() {
    ...
    # Even models-only and L.P.S./Desert selections are user-confirmed actions.
    # Full-stack normal models get this via install_odysseus instead.
    if [[ "$scope" == '--models-only' || "$have_normal" != 1 ]]; then
        ensure_docker_for_committed_install || echo '    [warning] Docker setup incomplete; continuing selected models.'
        ensure_chromadb_running || true
    fi
    ...
    for record in "${items[@]}"; do
        ...
        case "$family" in
            ...
            desert)
                ...
                ;;
            normal)
                if [[ "$kind" == text && ( "${repo,,}" == tobestyledintro/qwen3.8-9b-distill || "${repo,,}" == tobestyledintro/qwen3.8-9b-distill:* ) ]]; then
                    if pull_model_auto "ollama:$repo"; then complete=$((complete+1)); else failed=$((failed+1)); fi
                elif [[ "$kind" == audio && ( "$task" == automatic-speech-recognition || "$task" == text-to-speech ) ]]; then
...
tui_network_menu() {
    local choice
    while true; do
        if _tui_menu 'NETWORK DISCOVERABILITY' \
            'LOCALHOST ONLY (127.0.0.1)' \
            'LOCAL NETWORK / ALL INTERFACES (0.0.0.0)' \
            'SHOW CURRENT MODE AND LAN IP ADDRESSES' \
            'BACK'; then
            choice="$TUI_CHOICE"
            case "$choice" in
                0) _tui_run_command 'Setting localhost-only listener' network-mode localhost ;;
                1) _tui_run_command 'Enabling LAN listener (trusted networks only)' network-mode lan ;;
                2) _tui_run_command 'Odysseus network status and addresses' network-mode status ;;
                3) return 0 ;;
            esac
        else return 0; fi
    done
...
tui_ollama_menu() {
    ...
    while true; do
        if _tui_menu 'OLLAMA MANAGEMENT' \
            ...
            'TOOL CALLING HEALTH' \
            'INSTALL QWEN EXPLICIT TOOL FOCUS' \
            'PULL MODEL            [opull]' \
            ...
            case "$choice" in
                ...
                7) _tui_run_command 'Tool-calling health' tool-health ;;
                8) _tui_run_command 'Installing Qwen explicit-tool focus' agent-tool-focus-install ;;
                9) tui_opull ;;
                10)
...
case "$ACTION" in
    tui)
        ...
        ;;
    network-mode)
        if [[ "${2:-status}" == status ]]; then network_status; else network_mode_set "${2:-}"; fi
        ;;
    repo-update)
        if repo_installed; then
            refresh_existing_odysseus_checkout
    ...
    workspace-default)
        if [[ "${2:-}" == status ]]; then
            echo "Detected project checkout: $PROJECT_DIR"
            if [[ -f "$PROJECT_DIR/static/js/workspace.js" ]] && grep -Fq 'ZARZYSSEUS_AGENT_WORKSPACE_DEFAULT_V1' "$PROJECT_DIR/static/js/workspace.js"; then
                echo 'Default Agent workspace patch: installed (browser selections can override)'
            else
                echo 'Default Agent workspace patch: not installed'
            fi
        else
            [[ -z "${2:-}" ]] || { echo "Usage: $0 workspace-default [status]"; exit 2; }
            ensure_odysseus_default_workspace
        fi
        ;;
    tool-health)
        ...
        ;;
    docker-setup)
        ensure_docker_for_committed_install force
        ensure_chromadb_running force
        ;;
    chromadb-start)
        ensure_chromadb_running force
        ;;
    agent-tool-focus-install)
        application_installed || { echo "ERROR: Install Odysseus first."; exit 1; }
        install_qwen_tool_focus
        if service_installed && service_systemctl is-active --quiet "$service_unit" 2>/dev/null; then
            service_systemctl restart "$service_unit"
            echo 'Zarzysseus restarted with Qwen tool focus.'
        fi
        ;;
    accelerator-status)
        accelerator_status
...
Usage:
  ...
  $0 workspace-default [status]  Make the discovered/cloned checkout Agent's first-use workspace
  $0 network-mode localhost|lan|status  Persist localhost or 0.0.0.0 binding; show LAN URLs
