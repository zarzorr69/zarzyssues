...
ensure_chromadb_running() {
    [[ "$CHROMADB_AUTO_START" == 1 ]] || { echo '    [note] ChromaDB auto-start disabled.'; return 0; }
    ...
    if ! (cd "$PROJECT_DIR" && docker compose config --services 2>/dev/null | grep -Fxq chromadb); then
        echo '    [warning] This checkout has no chromadb Compose service.'
        return 1
    fi
    echo '==> Starting existing Odysseus ChromaDB service...'
    if ! (cd "$PROJECT_DIR" && docker compose up -d chromadb); then
